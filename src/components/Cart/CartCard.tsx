...
const CartCard = ({ item, quantity, updateQuantity }: CartCardProps) => {
    ...
    const image = mockProducts.find(product => product.variants.includes(item))?.featuredAsset.preview;
    ...
    return <CartCardContainer>
        ...
        <QuantityDropdown quantity={quantity} setQuantity={(quantity) => updateQuantity(item, quantity)} />
    </CartCardContainer>
}
